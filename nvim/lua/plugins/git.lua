  ...
  -- Git signs in the sign column
  {
    ...
    opts = {
      ...
      on_attach = function(bufnr)
        ...
        vim.keymap.set('n', '<leader>tl', require('gitsigns').toggle_linehl,
          { buffer = bufnr, desc = '[T]oggle [L]ine Highlights' })
        vim.keymap.set('n', '<leader>tL', require('gitsigns').toggle_deleted,
  ...
  -- View git changes in a diff view
  {
    ...
    config = function()
      ...
      vim.keymap.set("n", "<leader>gd", "<CMD>DiffviewOpen<CR>",
        { desc = '[G]it [D]iff', silent = true })

      -- Opens the diff for all changes relative to main/master branch
      vim.keymap.set("n", "<leader>gm", function() vim.cmd('DiffviewOpen ' .. get_default_branch_name()) end,
        { desc = '[G]it Diff [M]aster', silent = true })
      ...
      vim.keymap.set("n", "<leader>gF", "<CMD>DiffviewFileHistory --follow %<CR>",
        { desc = '[G]it Current [F]ile History', silent = true })

      -- Opens the diff for the current visual selection
      vim.keymap.set("v", "<leader>gl", "<ESC><CMD>'<,'>DiffviewFileHistory --follow<CR>",
